...
class Admin extends AdminModule
{
  ...
  public function anyTaskID()
  {
    ...
    $dokter_where = '';
    if (!empty($kd_dokter)) {
      $dokter_where = " AND reg_periksa.kd_dokter = ? ";
    }
    ...
    foreach ($query as $q) {
      ...
      $q['nomor_referensi'] = isset_or($mlite_antrian_referensi['kodebooking'], '');
  ...
  public function anyTaskIDMassal()
  {
    ...
    if (!empty($kd_dokter)) {
      $exclude_taskid = str_replace(",", "','", $this->settings->get('jkn_mobile.exclude_taskid'));
      $query = $this->db()->pdo()->prepare("SELECT pasien.no_rkm_medis, pasien.nm_pasien, reg_periksa.no_rawat, reg_periksa.tgl_registrasi, reg_periksa.jam_reg, reg_periksa.kd_dokter, dokter.nm_dokter, poliklinik.nm_poli, mar.kodebooking, mar.nomor_referensi
          FROM reg_periksa
          INNER JOIN pasien ON reg_periksa.no_rkm_medis = pasien.no_rkm_medis
          INNER JOIN dokter ON reg_periksa.kd_dokter = dokter.kd_dokter
          INNER JOIN poliklinik ON reg_periksa.kd_poli = poliklinik.kd_poli
          LEFT JOIN mlite_antrian_referensi mar ON mar.tanggal_periksa = reg_periksa.tgl_registrasi AND mar.no_rkm_medis = reg_periksa.no_rkm_medis
      ...
      foreach ($query as $q) {
        $nomor_referensi = isset_or($q['kodebooking'], '');
        if (empty($nomor_referensi)) {
          $nomor_referensi = isset_or($q['nomor_referensi'], '');
        }

        $default_datetime = $q['tgl_registrasi'] . ' ' . $q['jam_reg'];
        $q['nomor_referensi'] = $nomor_referensi;
        $q['task1'] = $default_datetime;
        $q['task2'] = $default_datetime;
        $q['task3'] = $default_datetime;
        $q['task4'] = $default_datetime;
        $q['task5'] = $default_datetime;
        $q['task6'] = $default_datetime;
        $q['task7'] = $default_datetime;
        ...
        if (!empty($nomor_referensi)) {
          for ($i = 1; $i <= 7; $i++) {
            $task = $this->db('mlite_antrian_referensi_taskid')
              ...
              ->oneArray();
            if (!empty($task) && !empty($task['waktu'])) {
              $q['task' . $i] = date('Y-m-d H:i:s', $task['waktu'] / 1000);
            }
          }
        }
    ...
    return $this->draw('taskid.massal.html', [
      'rows' => $rows,
      'has_rows' => !empty($rows),
  ...
  public function postSaveTaskIDMassal()
  {
    $tanggal_antrol = isset_or($_POST['tanggal_antrol'], date('Y-m-d'));
    $kd_dokter = isset_or($_POST['kd_dokter'], '');
    $nomor_referensi = isset_or($_POST['nomor_referensi'], []);

    if (empty($nomor_referensi)) {
      $this->notify('failure', 'Tidak ada data pasien untuk disimpan.');
      redirect(url([ADMIN, 'jkn_mobile', 'taskidmassal']), ['tanggal_antrol' => $tanggal_antrol, 'kd_dokter' => $kd_dokter]);
    }

    $pdo = $this->db()->pdo();
    $delete_stmt = $pdo->prepare("DELETE FROM mlite_antrian_referensi_taskid WHERE nomor_referensi = ? AND taskid IN ('1','2','3','4','5','6','7') AND (status IS NULL OR status <> 'Sudah')");
    $check_sudah_stmt = $pdo->prepare("SELECT 1 FROM mlite_antrian_referensi_taskid WHERE nomor_referensi = ? AND taskid = ? AND status = 'Sudah' LIMIT 1");
    $insert_stmt = $pdo->prepare("INSERT INTO mlite_antrian_referensi_taskid (tanggal_periksa, nomor_referensi, taskid, waktu, status, keterangan) VALUES (?, ?, ?, ?, 'Belum', 'Update massal taskid')");

    $saved = 0;
    $skipped = 0;
    $total_rows = count($nomor_referensi);

    for ($idx = 0; $idx < $total_rows; $idx++) {
      $noref = trim(isset_or($nomor_referensi[$idx], ''));
      if ($noref == '') {
        $skipped++;
        continue;
      }

      $delete_stmt->execute([$noref]);

      for ($task = 1; $task <= 7; $task++) {
        $task_key = 'task' . $task;
        $task_values = isset_or($_POST[$task_key], []);
        $task_time = trim(isset_or($task_values[$idx], ''));

        if ($task_time == '') {
          continue;
        }

        $task_timestamp = strtotime($task_time);
        if ($task_timestamp === false) {
          continue;
        }

        $check_sudah_stmt->execute([$noref, (string)$task]);
        $already_sent = $check_sudah_stmt->fetch(\PDO::FETCH_ASSOC);
        if ($already_sent) {
          continue;
        }

        $insert_stmt->execute([$tanggal_antrol, $noref, (string)$task, (string)($task_timestamp * 1000)]);
        $saved++;
      }
    }

    $this->notify('success', 'Update massal selesai. Data task tersimpan: ' . $saved . ', pasien tanpa referensi dilewati: ' . $skipped . '.');
    redirect(url([ADMIN, 'jkn_mobile', 'taskidmassal']), ['tanggal_antrol' => $tanggal_antrol, 'kd_dokter' => $kd_dokter]);
  }
  ...
  public function getTaskIDInput($no_rawat)
  {
    ...
    $taskid['99'] = date('Y-m-d H:i:s', isset_or($taskid99['waktu'], 0) / 1000);
  ...
  {
    $this->db('mlite_antrian_referensi_taskid')->where('nomor_referensi', $_POST['nomor_referensi'])->where('status', '!=', 'Sudah')->delete();

    if (!empty($_POST['taskid99'])) {
      $this->db('mlite_antrian_referensi_taskid')
        ->save([
          'tanggal_periksa' => $_POST['tgl_registrasi'],
          'nomor_referensi' => $_POST['nomor_referensi'],
          'taskid' => 99,
          'waktu' => strtotime($_POST['taskid99']) * 1000,
          'status' => 'Belum',
          'keterangan' => ''
        ]);
    } else {
  ...
  public function postHapusAntrol()
  {
    ...
    if (!$booking_registrasi) {
      ...
      $json = json_decode($output, true);
      $notif = json_encode($json);
      if ($json == NULL) {
        $notif = 'Data Booking di JKN Mobile Tidak Ada!';
        ...
        $this->db('mlite_antrian_referensi')->where('kodebooking', $_POST['kodebooking'])->update([
          'status_kirim' => 'Batal',
          'keterangan' => $_POST['keterangan']
        ]);
